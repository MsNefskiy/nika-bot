        ...
        try {
            ...
            if (interaction.isStringSelectMenu()) {
                if (interaction.customId === 'select_event_type') {
                    // Удаляем сообщение с выбором типа перед показом модалки
...
async function joinSlot(interaction: any, slot: string) {
    ...
    await (prisma.tribune as any).update({
        where: { id: tribune.id },
        data: { [slot]: interaction.user.id }
    });
...
async function handleLeaveRequest(interaction: any) {
    ...
    if (userSlots.length === 1) {
        ...
        const select = new StringSelectMenuBuilder()
            .setCustomId('select_leave_slot')
            .setPlaceholder('Выберите место для освобождения')
            .addOptions(userSlots.map(s => new StringSelectMenuOptionBuilder().setLabel(`Место ${s.replace('slot','').replace('_','.')}`).setValue(s)));
...
async function leaveSpecificSlot(interaction: any, slot: string) {
    ...
    if (interaction.isStringSelectMenu()) {
        await interaction.update({ content: 'Запись отменена!', components: [] });
        // После авто-удаления мусорных сообщений это сообщение исчезнет через 5 сек (логика ниже)
    } else {
        await updateTribuneMessage(interaction);
    }
}
...
async function completeTribune(interaction: any) {
    ...
    await prisma.tribuneHistory.create({
        data: {
            type: activeTribune.type,
            startTime: activeTribune.dateTime,
            hostId: interaction.user.id,
            participants: hostIds.join(',')
        }
    });
...
async function viewHistory(interaction: any, personal: boolean) {
    ...
    const embed = new EmbedBuilder()
        .setTitle(personal ? '🌟 Твои проведенные трибуны' : '📜 Общая история трибун')
    ...
    history.forEach(h => {
        ...
        const hosts = h.participants?.split(',')
            .map(id => id.trim())
            .filter(id => id && id !== 'null' && id !== 'пусто')
            .map(id => `<@${id}>`).join(', ') || '*нет ведущих*';
...
async function showEventSelector(interaction: any) {
    const select = new StringSelectMenuBuilder()
        ...
        .setPlaceholder('Выберите тип события')
        .addOptions(
